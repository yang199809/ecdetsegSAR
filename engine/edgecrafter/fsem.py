...
class ObjectAwareWaveletBranch(nn.Module):
    ...
    def forward(self, x, return_aux=False):
        ...
        if return_aux:
            return y, {"object_map": object_map, "band_weight": band_weight}
        return y
...
class OSAFSEMBlock(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_branch = SpatialBranch(dim, act=act)
        self.wavelet_branch = ObjectAwareWaveletBranch(dim, act=act)
        self.global_frequency_branch = GlobalFrequencyBranch(dim, base_fft_size)
        if use_fsas:
            self.fsas_branch = FSASBranch(dim, act=act)

        fuse_in = dim * (4 if use_fsas else 3)
        self.fusion = ConvNormLayer_fuse(fuse_in, dim, 1, 1, padding=0, act=None)
